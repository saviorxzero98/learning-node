...
import { SqliteQueryBuilder } from './sqlQueryBuilder';
...
db.serialize(function () {
    ...
    for (var i = 0; i < sampleData.length; i++) {
        let insertSqlBuilder = new SqliteQueryBuilder('INSERT INTO MyProfire (UserId, UserName, IsAdmin, CreateDate) VALUES (@UserId, @UserName, @IsAdmin, @CreateDate);');
    ...
    // Where
    let whereSqlBuilder = new SqliteQueryBuilder('SELECT * FROM MyProfire WHERE UserName=@UserName AND CreateDate=@CreateDate')
    ...
    // Update
    let updateSqlBuilder = new SqliteQueryBuilder('UPDATE MyProfire SET UserName = @NewUserName WHERE UserName = @UserName;');
